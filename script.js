...
document.getElementById("correspondenciaForm").addEventListener("submit", function (e) {
  ...
  doc.rect(10, 160, 190, 30); // Ampliado a 50 para incluir instructivo
  doc.text("CUARTO DESTINATARIO:", 12, 165);
  doc.setFont("helvetica", "normal");
  doc.text(`${destinatarioNombre} - ${cargoDestinatario}`, 65, 165);
  doc.setFont("helvetica", "bold");
  doc.text("INSTRUCTIVO:", 12, 173);
  doc.setFont("helvetica", "normal");
  const instructivoTexto = doc.splitTextToSize(instructivo, 185);
  doc.text(instructivoTexto, 12, 178);
